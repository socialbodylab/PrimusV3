...
_parse_list_line broke twice in V3.6:
- commit 9a104fb: SimpleFTPServer's 8-field format (no group column) was
  rejected, silently dropping every entry ("No WAV files on SD card")
- commit 07c4c8e: split(None, 7) produced filenames like "2000 HELLO.WAV"
  with the time/year field glued on
