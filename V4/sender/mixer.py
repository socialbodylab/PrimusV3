...
from effects import (
    EFFECTS, fx_none, compute_anim_factor, blend_pixels,
    ...
    for track in look.get("tracks", []) or []:
        ...
        for segment in track.get("segments", []) or []:
            if not isinstance(segment, dict):
                continue
            if "brightness_override" not in segment:
                segment["brightness_override"] = None
            elif segment["brightness_override"] is not None:
                segment["brightness_override"] = normalize_brightness(
                    segment["brightness_override"]
                )
    return look
...
def load_look(look_id):
    ...
    try:
        with open(path, "r") as f:
            return _with_look_defaults(json.load(f))
...
def list_looks(sort_by="modified"):
    ...
    for fname in os.listdir(d):
        ...
        try:
            ...
            continue
        looks.append(_with_look_defaults(look))
...
def _compute_segment_pixels(segment, local_t, pixel_count, grid, dt=0.033,
    ...
    return scale_pixels(pixels, brightness)
...
def compute_look_frame(look, t, fps=30, clip_cache=None, state_cache=None):
    ...
    if look_brightness < 1.0:
        result = [scale_pixels(pixels, look_brightness) if pixels else []
