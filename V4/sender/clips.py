...
from effects import normalize_brightness
...
def _with_clip_defaults(clip):
    for key, value in DEFAULT_CLIP.items():
        clip.setdefault(key, _default_value(value))
    clip["brightness"] = normalize_brightness(
        clip.get("brightness", DEFAULT_CLIP["brightness"])
    )
    return clip
...
def new_clip(name, output_type, group=None, **overrides):
    ...
    clip = {
        ...
        **{k: _default_value(v) for k, v in DEFAULT_CLIP.items()},
    ...
    for k, v in overrides.items():
        if k in clip:
            clip[k] = v
    return _with_clip_defaults(clip)
...
def load_clip(clip_id):
    ...
    try:
        with open(path, "r") as f:
            return _with_clip_defaults(json.load(f))
...
def compute_clip_preview(clip, t, dt=0.033):
    ...
    from effects import EFFECTS, fx_none, compute_anim_factor, scale_pixels
