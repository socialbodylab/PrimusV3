...
document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("lookMixer", () => ({
        ...
        _designerLiveInterval: null,
        _designerLivePending: false,
        ...
        normalizeBrightnessOverride(value) {
            if (value === null || value === undefined || value === "") return null;
            return this.normalizeBrightness(value);
        },

        brightnessPercent(value) {
            return Math.round(this.normalizeBrightness(value) * 100);
        },

        normalizeLook(look) {
            if (!look) return look;
            look.brightness = this.normalizeBrightness(look.brightness);
            for (const track of look.tracks || []) {
                for (const seg of track.segments || []) {
                    seg.brightness_override = this.normalizeBrightnessOverride(seg.brightness_override);
                }
            }
            return look;
        },
        ...
        designerReceiverFpsLabel() {
            const connected = this.devices.filter(dev => dev.connected);
            if (!connected.length) return "No devices";
            const values = connected
                .map(dev => dev.receiver_fps)
                .filter(value => value !== null && value !== undefined);
            if (!values.length) return "-- fps";
            if (values.length === 1) return values[0] + " fps";
            const min = Math.min(...values);
            const max = Math.max(...values);
            return min === max ? min + " fps" : min + "-" + max + " fps";
        },
        ...
        async init() {
            ...
            document.addEventListener('primus:mode-changed', (event) => {
                if (event.detail?.mode !== 'mixer') {
                    ...
                    if (this.subMode === 'designer') {
                        this.startDesignerLiveLoop();
                        this.ensureDesignerLive();
                    }
        ...
        async setSubMode(m) {
            ...
            if (m === "designer") {
                await this.ensureDesignerLive();
            } else {
                await api("POST", "/api/set_playback_source", { source: "idle" });
            }
        },

        startDesignerLiveLoop() {
            if (this._designerLiveInterval) return;
            this._designerLiveInterval = setInterval(() => {
                if (this.subMode !== 'designer' || Alpine.store('app').mode !== 'mixer') return;
                if (this.playbackInfo.source !== 'designer') this.ensureDesignerLive();
            }, 1500);
        },

        stopDesignerLiveLoop() {
            if (this._designerLiveInterval) {
                clearInterval(this._designerLiveInterval);
                this._designerLiveInterval = null;
            }
            this._designerLivePending = false;
        },

        async ensureDesignerLive() {
            if (this._designerLivePending) return;
            if (this.subMode !== 'designer' || Alpine.store('app').mode !== 'mixer') return;
            this._designerLivePending = true;
            try {
                await api("POST", "/api/set_playback_source", { source: "designer" });
                await Alpine.store("app").fetchState();
            } finally {
                this._designerLivePending = false;
            }
        ...
        async loadClips() {
            this.allClips = (await api("GET", "/api/clips")).map(clip => ({
                ...clip,
        ...
        openImportFile() {
            this.$refs.importBundleInput?.click();
        },
        ...
        exportClip(clip) {
            if (!clip?.id) return;
            this.downloadExport("/api/clips/" + encodeURIComponent(clip.id) + "/export");
        },

        exportLook(look = this.look) {
            if (!look?.id) {
                Alpine.store("app").showNotice("Save the Look before exporting it.", "info");
                return;
            }
            this.downloadExport("/api/looks/" + encodeURIComponent(look.id) + "/export");
        },
        ...
        async importBundleFile(event) {
            ...
            try {
                ...
                const result = await api("POST", "/api/import_bundle", bundle);
                await this.loadClips();
                await this.refreshLibrary();

                if (result.kind === "look" && result.look) {
                    if (this.previewing) await this.stopPreview();
                    if (this.subMode !== "timeline") await this.setSubMode("timeline");
                    this.look = this.normalizeLook(result.look);
                    this.playTime = 0;
                    ...
                    document.dispatchEvent(new CustomEvent('primus:looks-changed', {
                        detail: { look: result.look },
                    }));
                    Alpine.store("app").showNotice(
                        "Imported Look with " + (result.clips_imported || 0) + " Clips.",
                        "success"
                    );
                } else if (result.kind === "clip" && result.clip) {
                    Alpine.store("app").showNotice("Imported Clip " + result.clip.name + ".", "success");
                } else {
                    Alpine.store("app").showNotice("Import complete.", "success");
                }
            } catch (e) {
                Alpine.store("app").showApiError("Import failed", e);
            } finally {
                input.value = "";
            }
        },
        ...
        updateOutputBrightness(oi, value) {
        ...
        async doSaveClip() {
            ...
            const clip = {
                ...
                speed: out.speed,
                brightness: this.normalizeBrightness(out.brightness),
        ...
        openEditSegment(trackIdx, segIdx) {
            ...
            this.editData = {
                ...
                brightness_override: this.normalizeBrightnessOverride(seg.brightness_override),
            };
            const track = this.look.tracks[trackIdx];
        ...
        applyEditSegment() {
            ...
            seg.brightness_override = this.normalizeBrightnessOverride(this.editData.brightness_override);
            this._mixerFrameDirty = true;
            this.editModal = false;
            if (this.previewing) this._sendPreview();
        },
        ...
        _sendPreview() {
            ...
            const payload = { ...this.normalizeLook(this.look) };
        ...
        updateLookBrightness(value) {
            if (!this.look) return;
            this.look.brightness = this.normalizeBrightness(Number(value) / 100);
            this._mixerFrameDirty = true;
            if (this.previewing) this._sendPreview();
        },
        ...
        async loadLook(id) {
            ...
            if (look) {
                ...
                this.look = this.normalizeLook(look);
                this.playTime = 0;
                this.transportTime = 0;
                this.selectedTrackIdx = 0;
                this.clearSelectedSegment();
