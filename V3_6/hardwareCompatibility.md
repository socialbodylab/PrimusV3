# V3.6 Hardware Compatibility

V3.6 uses one shared firmware source tree with separate Arduino build profiles. V1 and V2 boards must be reflashed with V3.6 firmware; the V3.6 sender does not use the old V1 OSC protocol or the old V2 brightness-byte Art-Net payload.

Dynamic brightness is handled in the sender by scaling outgoing RGB pixels. Receiver LED drivers stay locked at brightness 255 for every hardware profile, which keeps all output types on the same standard ArtDmx data path and avoids the flicker risk from runtime firmware brightness changes.
...
All V3.6 profiles advertise the existing `PV3CAP1` Node Report contract with one compatible extension:
