...
python3 V5/tools/osc_cue_sender/run.py --port 8105 --no-browser
```
...
1. Run **one** PrimusCentral instance (packaged app or dev sender):
   ...
   # Dev:
   python3 V5/sender/run.py --product primus --no-browser --port 8080
   ```
...
- macOS: `~/Library/Application Support/PrimusV3/V5/tools/osc_cue_sender/`
- Windows: `%APPDATA%\PrimusV3\V5\tools\osc_cue_sender\`
...
# Windows .exe
python3 V5/tools/build_app.py --target windows

# Unsigned console build (debug)
python3 V5/tools/build_app.py --console --onedir
```

Output: `V5/tools/dist/macos/OscCueSender.app` or `V5/tools/dist/windows/OscCueSender.exe`
...
```bash
python3 -m unittest discover -s V5/tools/osc_cue_sender/tests
python3 -m py_compile V5/tools/osc_cue_sender/*.py
```
