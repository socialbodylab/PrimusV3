...
class LanePortCapabilityTests(unittest.TestCase):
    ...
    def test_legacy_radius_without_aud_defaults_to_show_radius(self):
        # Pre-lane Radius firmware (no advertised AUD/SHOW lane) runs
        # ArtAudio/FTP/management on its own port 6456 (config.h
        # PORT_SHOW_DEFAULT), off the 6454 discovery/LED port. Verified on real
        # hardware: routing these to 6454 makes the node go silent.
