...
class ReceiveModeHelperTests(unittest.TestCase):
    ...
    def test_split_universes_are_stable_when_a0_is_off(self):
        outputs = [
            {"name": "A0", "type": "none", "count": 0, "universe": 0},
            {"name": "A1", "type": "long_strip", "count": 72, "universe": 1},
        ]
        _apply_output_universes(outputs, "split", 20)
        self.assertEqual(outputs[0]["universe"], 20)
        self.assertEqual(outputs[1]["universe"], 21)

        _apply_output_universes([outputs[1]], "split", 30)
        self.assertEqual(outputs[1]["universe"], 31)
...
class SetDeviceReceiveModeTests(unittest.TestCase):
    ...
    @patch("state.send_receive_config")
    def test_set_device_receive_mode_rejects_split_base_32767(
        self, send_receive_config
    ):
        result = self.state.set_device_receive_mode(0, "split", 32767)
        self.assertFalse(result["ok"])
        self.assertIn("0-32766", result["error"])
        send_receive_config.assert_not_called()
