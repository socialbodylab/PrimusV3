#!/usr/bin/env python3
"""Build a one-click V5 sender app with PyInstaller.

V5 is the unified sender tree (Primus + Radius). Build on the target OS: macOS builds a `.app`, while Windows builds an `.exe`.
...
def _data_files(v5_dir, sender_dir, product="radius"):
    files = [
        (v5_dir / "Arduino", "Arduino"),
...
def _build_command(args, sender_dir, build_dir, dist_dir, icon_path=None):
    ...
    v5_dir = sender_dir.parent
    for source, dest in _data_files(v5_dir, sender_dir, product=args.product):
        if source.exists():
...
def _prepare_windows_readme(v5_dir, dist_dir):
    source = v5_dir / WINDOWS_README_SOURCE
    if not source.exists():
...
def main(argv=None):
    ...
    repo_root = v5_dir.parent
    sender_dir = v5_dir / "sender"
    build_dir = v5_dir / "build" / args.target
    dist_dir = v5_dir / "dist" / args.target
    ...
    if importlib.util.find_spec("PyInstaller") is None:
        print("PyInstaller is not installed. Install build tools with: python -m pip install -r V5/requirements-build.txt")
        return 1
